# Serial Protocol Reference — v3 Boot Loader (Complete Command Set)
...
> **Scope: this is the protocol implemented by the v3 boot loader.** Everything
> below describes the command set of the **version 3** boot ROM (BIOS
> version 1.20, as reported by the `I` and `V` commands — see §8). A different
> boot loader revision may add, remove or change commands; if your machine
> reports a different BIOS version, treat this document as a starting point
> rather than an exact match.

It complements the two existing documents:

- [SerialProtocol.md](SerialProtocol.md) — the original field notes, describing
  the commands that were observed on the wire.
- [HighLevel.md](HighLevel.md) — the higher-level operations (embroidery
  sessions, file transfer) built on top of these primitives.

Everything here was reconstructed from the **v3 boot ROM**. The authoritative
source is the rewritten-in-C boot ROM at
`sim_h83003/bernina_artista180/bootrom/boot.c` — in particular the command
dispatcher `state_idle()` and the state machine `serial_service()`.
...
> roughly 14,750 times through its main loop (boot ROM vector *slot 1*). So this
> command set is live **both** while the boot ROM is waiting for firmware **and**
> while the normal sewing/embroidery application is running. This is why a host
> can read memory, checksum, and write while the machine is in normal use.
...
The machine has **two** serial channels (SCI0 and SCI1). SCI1 is the PC port;
SCI0 reaches the embroidery module. The boot ROM's `T` command and the internal
port bridge decide which one your bytes reach. For ordinary use you talk to the
PC port and everything below applies as written.
...
[SerialProtocol.md](SerialProtocol.md)).
...
> see [HighLevel.md](HighLevel.md) for the session-level view.
...
```
Send:     I
Receive:  <echo> "BERNINA Electronic AG\r" "BiosVersion: 1.20\r" "July 97\r"
```

Three CR-terminated strings. Handy as a human-readable "who are you".
...
- `XX` — the BIOS version byte as 2 hex characters. `0C` corresponds to the
  "1.20" reported by `I`.
...
1. Brings the bus and both serial channels up at **19200**.
2. Sends `BOS` (0x42 0x4F 0x53).
3. Offers the link to a host for a fixed window, polling both channels.
...
- On the PC port (SCI1) each matched character is echoed; a wrong byte draws a
  ...
  `Q` to stray bytes).
- Matching `EB` claims the link on whichever channel completed the match.
...
```
I                -> banner: BERNINA Electronic AG / BiosVersion: 1.20 / July 97
R200100          -> firmware version/language/manufacturer strings (32 bytes hex)
V                -> BIOS version byte (e.g. "0C")
```
...
```
EB   (claim) -> J05 -> BOS (57600) -> EB (re-claim, quickly)
```
...
Link        J DD                -> BOS (new rate), re-handshake
            ...
            EB                  -> claim link (handshake)

Info        I                   -> identity banner
            V                   -> version byte (hex)
            K                   -> O                     (ping)
